...
moments, is the job of an **overlay manager**. The best way to see how this
works in practice is a real, large program: the SBC's own C compiler.

### A worked example: the SMALLC99 compiler
...
The full design, including exactly how the expression code is split and how
the parts call each other, is described in
[SMALLC99: overlays in practice](#smallc99-overlays-in-practice) below.
...
and so on — see the overlay table below).
...
that SMALLC99's larger manager builds on.
...
## SMALLC99: Overlays in Practice

SMALLC99 uses the same mapper, the same chain-block EXE and the same kind of
overlay manager, scaled up: a four-segment window instead of one segment,
overlays that span several pages, and overlays that call each other.
